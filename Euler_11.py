...
# Create a new grid filled with 1's
new_grid = [[1 for _ in range(width)] for _ in range(height)]

# Populate the inner portion of the new grid with original grid values
for i in range(3, height - 3):
    for j in range(3, width - 3):
        new_grid[i][j] = grid[i - 1][j - 1]

# Print the new grid to verify
for row in new_grid:
    print(row)
...
# Split the string into rows and then split each row into numbers
grid = np.array([[int(num) for num in row.split()] for row in grid_str.strip().split('\n')])

# Add a border of 1's around the grid
